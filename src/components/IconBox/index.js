'use client'

import DOMPurify from 'dompurify'
import React from 'react'
import { useRouter } from 'next/navigation'
import { sanitizeLink } from '@/utils/sanitizeLink'
import Link from 'next/link'
import Image from 'next/image'

const IconBox = ({ data, spaceBefore, spaceAfter, id }) => {
  const router = useRouter()
  return (
    <section
      id={id}
      className={`list-icon-box ${
        data.centerAlignment === '0'
          ? 'align-items-start'
          : 'align-items-center text-center'
      } ${spaceBefore && `frame-space-before-${spaceBefore}`} ${
        spaceAfter && `frame-space-after-${spaceAfter}`
      } ${
        data.hoverVariation === '0'
          ? 'icon-box-white'
          : data.hoverVariation === '1'
          ? 'icon-box-gradient'
          : data.hoverVariation === '2'
          ? 'icon-box-animated'
          : ''
      } ${
        data.boxBackgroundColor === '0' ? 'bg-light-color' : 'bg-gray-color'
      } ${
        data.horizontalAlignment === '0'
          ? 'flex-column'
          : 'flex-lg-row flex-column horizontal-icon'
      } ${data.boxBorder === '0' ? 'border-0' : ''}`}
      data-aos={data.selectAnimation && data.selectAnimation}
    >
      {data.icon && data.icon.length > 0 && data.icon[0].publicUrl && (
        <div
          className={`list-icon-box__icon ${
            data.iconBackgroundColor === '0'
              ? 'bg-light-color'
              : data.iconBackgroundColor === '1'
              ? 'bg-gray-color'
              : data.iconBackgroundColor === '2'
              ? 'bg-primary-color'
              : 'bg-none'
          } ${data.circleIcon === '1' ? 'rounded-circle' : ''} `}
        >
          <Image
            src={data.icon[0].publicUrl}
            alt={
              data.icon[0].properties?.alternative
                ? data.icon[0].properties?.alternative
                : 'image'
            }
            title={data.icon[0].properties?.title}
            height={0}
            width={0}
            sizes="100vw"
            style={{ width: 'auto', height: 'auto' }}
          />
        </div>
      )}
      <div className="list-icon-box__text">
        {data.header && <h4>{data.header}</h4>}
        <p
          onClick={(e) => {
            e.preventDefault(), sanitizeLink(e, router)
          }}
          dangerouslySetInnerHTML={{
            __html: DOMPurify.sanitize(data.text),
          }}
        />

        {data && data?.listBlock && data.listBlock.length > 0 && (
          <ul>
            {data?.listBlock?.map((item, index) => {
              return (
                <React.Fragment key={index}>
                  {item.list && (
                    <li
                      className={`list-icon-box__list ${
                        data.centerAlignment === '1'
                          ? 'justify-content-center'
                          : ''
                      }`}
                    >
                      {data.selectListIcon === '1' ? (
                        <svg
                          stroke="currentColor"
                          fill="currentColor"
                          strokeWidth="0"
                          viewBox="0 0 512 512"
                          className="check-icon"
                          height="1em"
                          width="1em"
                          xmlns="http://www.w3.org/2000/svg"
                        >
                          <path d="M173.898 439.404l-166.4-166.4c-9.997-9.997-9.997-26.206 0-36.204l36.203-36.204c9.997-9.998 26.207-9.998 36.204 0L192 312.69 432.095 72.596c9.997-9.997 26.207-9.997 36.204 0l36.203 36.204c9.997 9.997 9.997 26.206 0 36.204l-294.4 294.401c-9.998 9.997-26.207 9.997-36.204-.001z"></path>
                        </svg>
                      ) : (
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          viewBox="0 0 64 64"
                          width="64px"
                          height="64px"
                        >
                          <linearGradient
                            id="D8kn8RyCktjRlx36eKmJ5a"
                            x1="32"
                            x2="32"
                            y1="12.664"
                            y2="52.422"
                            gradientUnits="userSpaceOnUse"
                            spreadMethod="reflect"
                          >
                            <stop offset="0" stopColor="#1A6DFF" />
                            <stop offset="1" stopColor="#C822FF" />
                          </linearGradient>
                          <path
                            fill="url(#D8kn8RyCktjRlx36eKmJ5a)"
                            d="M24.982,51c-1.273,0-2.547-0.475-3.524-1.429L6.888,35.364C6.315,34.806,6,34.061,6,33.268 s0.315-1.538,0.889-2.097l2.82-2.75c1.166-1.137,3.063-1.137,4.228,0.001l10.259,10.003c0.395,0.385,1.058,0.38,1.446-0.012 l24.341-24.526c1.147-1.156,3.044-1.186,4.228-0.068l2.867,2.705c0.582,0.55,0.91,1.29,0.923,2.083 c0.013,0.793-0.291,1.542-0.854,2.109L28.565,49.514C27.584,50.504,26.283,51,24.982,51z M11.822,29.564 c-0.26,0-0.52,0.097-0.717,0.29l-2.82,2.75C8.101,32.783,8,33.018,8,33.268s0.102,0.485,0.285,0.664l14.569,14.208 c1.19,1.163,3.116,1.148,4.291-0.034l28.581-28.798c0.181-0.182,0.277-0.418,0.273-0.668c-0.004-0.25-0.109-0.485-0.296-0.661 l-2.867-2.705c-0.401-0.381-1.047-0.369-1.435,0.022L27.061,39.823c-1.166,1.173-3.079,1.189-4.263,0.034L12.54,29.853 C12.343,29.66,12.083,29.564,11.822,29.564z"
                          />
                          <linearGradient
                            id="D8kn8RyCktjRlx36eKmJ5b"
                            x1="32.013"
                            x2="32.013"
                            y1="16.83"
                            y2="47.526"
                            gradientUnits="userSpaceOnUse"
                            spreadMethod="reflect"
                          >
                            <stop offset="0" stopColor="#8ab4ff" />
                            <stop offset="1" stopColor="#e492ff" />
                          </linearGradient>
                          <path
                            fill="url(#D8kn8RyCktjRlx36eKmJ5b)"
                            d="M24.977,46.609c-0.489,0-0.98-0.181-1.368-0.544L10.318,33.603l1.367-1.459l13.292,12.461 L52.293,17.29l1.414,1.414L26.391,46.019C26,46.411,25.489,46.609,24.977,46.609z"
                          />
                        </svg>
                      )}
                      <span>{item.list}</span>
                    </li>
                  )}
                </React.Fragment>
              )
            })}
          </ul>
        )}
        {data.linkText && (
          <div className="icon-box-text__link">
            {data.selectLinkVariation === '0' ? (
              <Link href={data.link.href} className="icon-box-link">
                {data.linkText}{' '}
                <svg
                  stroke="currentColor"
                  fill="currentColor"
                  strokeWidth="0"
                  viewBox="0 0 16 16"
                  height="1em"
                  width="1em"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    fillRule="evenodd"
                    d="M1 8a.5.5 0 0 1 .5-.5h11.793l-3.147-3.146a.5.5 0 0 1 .708-.708l4 4a.5.5 0 0 1 0 .708l-4 4a.5.5 0 0 1-.708-.708L13.293 8.5H1.5A.5.5 0 0 1 1 8z"
                  ></path>
                </svg>
              </Link>
            ) : data.selectLinkVariation === '5' ? null : (
              <Link
                href={data.link.href}
                className={`btn ${
                  data.selectLinkVariation === '4'
                    ? 'btn__gradient'
                    : data.selectLinkVariation === '3'
                    ? 'btn__outline'
                    : data.selectLinkVariation === '2'
                    ? 'btn--secondary'
                    : ''
                }`}
              >
                {data.linkText}
              </Link>
            )}
          </div>
        )}
      </div>
    </section>
  )
}

export default IconBox
